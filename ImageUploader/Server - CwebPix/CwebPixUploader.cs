...
{
    public class CwebPixUploader : IUploader
    {
        ...
        private void fetchDataFromServerAndFillClassFields()
        {
            ...
            {
                string siteCode = client.DownloadString("https://www.cweb-pix.com");
                string cookie = client.ResponseHeaders["Set-Cookie"];
                int start = cookie.IndexOf("__cfduid=");
                int end = cookie.IndexOf(";", start);
                _cookie = cookie.Substring(start, end - start + 1);
                start = cookie.IndexOf("PHPSESSID=");
                end = cookie.IndexOf(";", start);
                _cookie += " " + cookie.Substring(start, end - start + 1);
                
                start = siteCode.IndexOf("PF.obj.config.auth_token = \"") + "PF.obj.config.auth_token = \"".Length;
                end = siteCode.IndexOf("\"", start);
        ...
        {
            // Headers
            HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create("https://www.cweb-pix.com/json");
            ...
            myRequest.Headers.Add("Cookie", _cookie + " _gat=1; _ga=GA1.2.1645475153.1502435165; _gid=GA1.2.629091392.1502435165");
